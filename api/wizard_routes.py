...
def save_wizard_trial():
    try:
        ...
        status_class = data.get('status_class', 'incomplete')
        status_label = data.get('status_label', 'Incomplete')
        ...
        cursor.execute("""
            INSERT INTO user_trials (
                user_id, status_class, status_label, saved_data, result_data, is_submitted, created_at, last_updated
        ...
        connection.commit()
        return jsonify({"success": True, "message": "Trial saved"}), 201

    except Exception as e:
        current_app.logger.error(f"Error saving wizard trial: {str(e)}")
        return jsonify({"success": False, "message": "Internal server error"}), 500
